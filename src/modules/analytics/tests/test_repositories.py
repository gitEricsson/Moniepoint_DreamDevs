...
──────────────────────────────────────────────────────────────────────────────
Unit tests for AnalyticsRepository and ActivityRepository.

Seeded data allows exact numeric assertions against every query.
"""
...
class TestActivityRepository:
    """Tests for the write-side repository."""
    ...
    async def test_bulk_insert_returns_count(self, db_session: AsyncSession):
        ...
        # Insert dicts (bypass SQLAlchemy ORM objects for bulk path)
        dicts = [
            {
                "event_id": r.event_id,
...
async def seeded_session(db_session: AsyncSession) -> AsyncSession:
    """Seed deterministic rows for analytics queries."""
    rows = [
        # MRC-000001 — highest volume, two products
        ...
        # FAILED — excluded from top-merchant & monthly-active, included in failure rate
...
class TestAnalyticsRepository:
    """Integration-style unit tests with seeded in-memory data."""
    ...
    async def test_monthly_active_merchants_counts(
        ...
        # Jan: MRC-000001, MRC-000002, MRC-000003 (has SUCCESS) = 3
        # Feb: MRC-000001 = 1
        # Mar: MRC-000004, MRC-000005 = 2  (KYC success rows)
    ...
    async def test_product_adoption_sorted_desc(
        ...
        # POS appears twice for different merchants
    ...
    async def test_failure_rates_formula(self, seeded_session: AsyncSession):
        """BILLS: 1 FAILED / (1 SUCCESS + 1 FAILED) = 50.0%"""
        repo = AnalyticsRepository(seeded_session)
